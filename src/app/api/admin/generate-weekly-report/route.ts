...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const week = body.week || 'Weekly Report';
    ...
    // For weekly report, simulate fewer reports (take first half for "this week")
    const weeklyReports = mockAdminReports.slice(0, Math.ceil(mockAdminReports.length / 2));

    // Build stats
    const pendingCount = weeklyReports.filter((r) => r.status === 'pending').length;
    const activeCount = weeklyReports.filter(
      (r) => r.status === 'dispatched' || r.status === 'acknowledged'
    ).length;
    const resolvedCount = weeklyReports.filter((r) => r.status === 'resolved').length;
    const driverNames = new Set(weeklyReports.map((r) => r.driver.driverName));
    ...
    // Return PDF
    const filename = `MDRRMO_Weekly_Report_${week.replace(/[\s,/]+/g, '_')}.pdf`;
